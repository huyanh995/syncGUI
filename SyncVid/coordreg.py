...
def prepare_dir():
    cmd = 'mkdir -p ~/.insightface/models/'
    subprocess.call(cmd, shell=True)
    cmd = 'cp models/scrfd_10g_bnkps.onnx ~/.insightface/models/'
    subprocess.call(cmd, shell=True)
    cmd = 'unzip models/2d106det.zip -d ~/.insightface/models/'
    subprocess.call(cmd, shell=True)
...
class Handler:
    ...
    def get(self, img, get_all=False):
        ...
        bboxes, _ = self.detector.detect(det_im,input_size =(192,192))
        if bboxes.shape[0] == 0:
            return out
        bboxes /= det_scale
        if not get_all:
