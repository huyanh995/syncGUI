...
_via_temporal_segmenter.prototype._init = function() {
  ...
  if ( this.group_aid_candidate_list.length ) {
    ...
    promise.then(function(new_attribute_aid) {
      ...
      this.is_init_success = true;
      // ToDo
      this.group['Concentrating (staring somewhere off screen, or with eyes closed)'] = [];
      this.gid_list.push('Concentrating (staring somewhere off screen, or with eyes closed)');
      this._tmetadata_gmetadata_update();
      this.group['Taking notes'] = [];
      this.gid_list.push('Taking notes');
      this._tmetadata_gmetadata_update();
      this.group['Using Calculator'] = [];
      this.gid_list.push('Using Calculator');
      this._tmetadata_gmetadata_update();
      this.group['Looking up something from the module (on phone, in book, or in notes)'] = [];
      this.gid_list.push('Looking up something from the module (on phone, in book, or in notes)');
      this._tmetadata_gmetadata_update();
      this.group['Phone call/Text Message'] = [];
      this.gid_list.push('Phone call/Text Message');
      this._tmetadata_gmetadata_update();
      this.group['Off task (general)'] = [];
      this.gid_list.push('Off task (general)');
      this._tmetadata_gmetadata_update();
      this.group['Distracted (general)'] = [];
      this.gid_list.push('Distracted (general)');
      this._tmetadata_gmetadata_update();
