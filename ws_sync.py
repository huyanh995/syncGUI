...
def ws_sync_call(screen_video, webcam_video, max_offset, trim):
    """
    Screen video as an anchor. Trim webcam video
    """
    # file2 is screen
    # First combine screen and audio, assume that audio is the same name, just different extension
    print("DEBUG: >>>>>", screen_video, webcam_video)
    base_name = os.path.basename(screen_video)
    dir_name = os.path.dirname(screen_video)
    file_name = os.path.splitext(base_name)[0]

    audio = screen_video.replace('avi', 'mp3')
    new_screen = "{}/{}.mp4".format(dir_name, file_name)
    print("DEBUG: >>>>>", audio, new_screen)
    cmd = 'ffmpeg -i {} -i {} -c:v copy -c:a aac {}'.format(screen_video, audio, new_screen)
    print("DEBUG: >>>>>", cmd)
    subprocess.call(cmd, shell=True)    
    ...
    offset, score, log = find_offset(new_screen, webcam_video, trim=trim, max_offset_in_seconds=max_offset)
    ...
    len1 = get_length(new_screen)
    len2 = get_length(webcam_video)
    ...
    '''
    if offset > 0:
        # print("File1 starts earlier than new_screen by {:0.3f} seconds. Sync score: {:0.2f}".format(offset, score))
        ss1 = offset
        ss2 = 0
        duration = min(len2, len1 - offset)
    else:
        offset = -offset
        # print("new_screen starts earlier than File1 by {:0.3f} seconds. Sync score: {:0.2f}".format(offset, score))
        ss1 = 0
        ss2 = offset
